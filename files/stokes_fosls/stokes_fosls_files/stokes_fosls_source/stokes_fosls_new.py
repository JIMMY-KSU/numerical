...
(STARTING CONDITION SPECIFIC BOUNDARY CONDITIONS)

is script uses the FEniCS project (primarily the DOLFIN functionality) in order
...
def main(n, tintervals, dt):
	...
	S2 = FunctionSpace(mesh, "CG", 1)
	S3 = FunctionSpace(mesh, "CG", 1)
	S4 = FunctionSpace(mesh, "CG", 1)
	S5 = FunctionSpace(mesh, "CG", 1)
	S6 = FunctionSpace(mesh, "CG", 1)
	...
	bc4 = DirichletBC(S.sub(5), noslip, top_bottom)
	bc5 = DirichletBC(S.sub(4), noslip, left_right)
